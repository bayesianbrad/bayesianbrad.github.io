...
## The Phase Space
 ...
 Is just a fancy coordinate system that is defined in terms of position and momentum, so when we explore trajectories in the phase space, we explore contours of energy, that is the <i> phase </i>  (angle with respect to position and momentum) changes, i.e the angle, but everything else remains fixed. 
