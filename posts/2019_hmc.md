---
layout: post
title:  "Hamiltonian Monte Carlo for beginners "
...
It turns out that this Irish guy named Hamilton, the guy behind the Hamiltonian, found a really cool way of representing such systems so that paths that explore the space efficiently naturally pop our from the representation.
...
## The Phase Space
 ...
 The phase space is just a fancy coordinate system that is defined in terms of position and momentum, so when we explore trajectories in the phase space, we explore contours of constant energy. That is the <i> phase </i> changes, i.e the angle, but everything else remains fixed, when the energy is constant. 

Here is an example phase space:
![png](/assets/posts/2018_hmc/phase_space.jpg)

The red inner regions represent where the motion is stable, i.e, we have energy preserving trajectories. 
This point is a integral reason for why HMC works so well in high-dimensions. It relates to something called [symplectic geometries](https://en.wikipedia.org/wiki/Symplectic_geometry), which essentially have lots of nice mathematically properties that just so happen to be the exact features we require for certain dynamical systems. <b>#ConspiracyTheory</b>. Because they preserve energy, we can define a MCMC strategy in which the chance of accepting a particle is high, by defining the acceptance criteria in terms of the energy! 

The regions outside of this red region are non-energy preserving, which means essentially our particle will stop moving eventually - not gouda for our MCMC algorithms, because this would lead to low-acceptance rates! Hence, we want to construct a system that is energy preserving. 

Thankfully, Hamilton provides us with a way to easily find these trajectories!
...
The Hamiltonian is the Legendre transform of the Lagrangian and gives us the total energy in the system. It is defined as follows: 
\begin{equation}
\label{one}\tag{1}
...
![png](/assets/posts/2018_hmc/potential.jpg)
...
<p>
$$\begin{align}
\dot{q} &= \dot{q} = M^{-1}p \label{position}\tag{5}\\ 
\dot{p} &= \dot{p}= -\frac{\partial U}{\partial q} \label{momentum}\tag{6}
\end{align}$$
</p>
...
![png](/assets/posts/2018_hmc/integrator.png)

<b>Because the Leapfrog integrator is volume preserving, it remains on the trajectories of interest!</b>
...
the kinetic energy, the $$d$$ momentum variables are independent, with $$p_i$$
having mean zero and variance $$m_i$$ in the multivariate case. 
...
tuned to obtain good performance. The momentum variables at the end of this $$L$$-step trajectory are then negated, giving a proposed state $$(q^∗, p^∗)$$.
