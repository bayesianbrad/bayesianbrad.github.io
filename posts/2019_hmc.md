...
## The Phase Space
 ...
 Is just a fancy coordinate system that is defined in terms of position and momentum, so when we explore trajectories in the phase space, we explore contours of energy, that is the <i> phase </i>  (angle with respect to position and momentum) changes in phase space! 
...
However, to implement numerical solver requires us to discretize Hamilton's equations. This induces errors **#Boo!**

##### The Leapfrog Integrator

The leapfrog integrator has properties that ensure that both global and local errors remain small **#Yay!**. In addition to this, the Leapfrog intergator has the property that it is volume preserving and thus allows us to dicretize Hamiltons equations **#OhLaLa**.
